...
MAINTAINER Sugesh Chandran <sugeshchandran@gmail.com>
...
RUN curl https://packages.grafana.com/gpg.key | apt-key add -\
 ...
 && apt-get -y update\
 && apt-get -y --allow-unauthenticated --allow-change-held-packages install vim\
 nginx\
 python-dev\
 python-flup\
 python-pip\
...
# install statsd
#RUN git clone -b tags https://github.com/DanCech/statsd.git /opt/statsd
